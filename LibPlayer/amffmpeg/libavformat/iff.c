...
static int parse_dsd_prop(AVFormatContext *s, AVStream *st, uint64_t eof)
{
    ...
    while (avio_tell(pb) + 12 <= eof) {
        ...
        case MKTAG('C','M','P','R'):
            if (size < 4)
                return AVERROR_INVALIDDATA;
            unsigned int tag = avio_rl32(pb);
            //st->codec->codec_id = ff_codec_get_id((const AVCodecTag *)dsd_codec_tags, tag);
            break;
